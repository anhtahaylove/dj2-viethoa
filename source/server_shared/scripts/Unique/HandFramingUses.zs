...
function previewIngredient(item as IItemStack, type as string) as IIngredient {
    return (item.withDisplayName("§r§6Khối Framing Template không bị tiêu hao.") | framingMaterial).marked(type).reuse();
}
...
function addInfo(stack as IItemStack) as IItemStack {
    return stack.withDisplayName("Tự tay đóng khung cho tủ!")
        .withLore([
            "Trên trái: mặt bên",
            "Trên phải: viền",
            "Giữa trái: mặt trước"
