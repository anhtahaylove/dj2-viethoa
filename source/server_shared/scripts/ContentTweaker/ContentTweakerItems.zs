...
unwarpification_talisman.itemRightClick = function(stack, world, player, hand) {
    ...
    Commands.call('/tellraw @p {"text":"Chỉ số Warp của bạn đã được đặt về 0!","bold":true}', player, world, false, true);
    return "Pass";
};
...
ritualistic_biome_checker.itemRightClick = function(stack, world, player, hand) {
    ...
    if(player.getDimension() != 0) {
        player.sendChat("Bạn phải ở Overworld để thực hiện công thức này!");
        return "PASS";
    }
    ...
    player.sendChat("Số biome Mortum khớp: " ~ mortumMatches ~ " / 24");
    player.sendChat("Số biome Hell khớp: " ~ hellMatches ~ " / 16");
    player.sendChat("Số biome Magical Forest khớp: " ~ magicalForestMatches ~ " / 8");
    player.sendChat("Số biome Ocean khớp: " ~ oceanMatches ~ " / 1");
