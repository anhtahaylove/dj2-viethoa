"""Resolve the term conflicts T2 wave 3 introduced.
...
BATCH = ROOT / "work" / "approved_t2_wave3_corpus.json"

# Adopt the shipped wording for the batch's own key.
ADOPT_SHIPPED = {
    # GuideAPI and JourneyMap both ship "Trước Đó" for the previous-page button.
    # Patchouli's lexicon has the same button, so it must read identically.
    ("patchouli", "patchouli.gui.lexicon.button.prev_page"): "Trước Đó",
    # AE2 ships "Mức Tạo Năng Lượng" for the network's energy generation rate;
    # ValkyrieLib's machine spec sheet labels the same quantity.
    ("valkyrielib", "gui.valkyrielib.energygen"): "Mức Tạo Năng Lượng",
}
...
def main() -> int:
    ...
    print(json.dumps({"changed": len(changed)}, ensure_ascii=False, indent=1))
    for line in changed:
        print("  ", line)
    return 0
